...
for times in range(10):
    ...
    for k in range(epi_num):
        ...
        #print(m)
        l_k1=math.floor((l_k*2)**(7/11)) #when m is unknown l_k1=math.floor((l_k*2)**((2*m+1)/(4*m-1))) #m is chosen via choose_m: in the end
        l_k1_total+=l_k1
        v_i=[]
        v_i_true=[]
        v_t=[]
        ...
        while t<=l_k:
            if t<=l_k1:  
                ...
                if t==l_k1:
                    ...
                    ker=Kernel(v_i_explore_true,y_t_explore,l_k1_total,1.3*l_k1_total**(-1/9)) 
                    ker_tt=Kernel(v_i_explore_true,y_t_explore,l_k1_total,1.3*l_k1_total**(-1/9))
                    print('--explore--')
                t=t+1
            else: #Exploitation
                ...
                rev_diff_tt=op_rev-solu.F(est_price_tt,theta_x) #our revenue difference
                ...
                rev_diff_mis=op_rev-solu.F(est_price_mis,theta_x) #revenue difference of RMLP-2
                reg.append(rev_diff)#exploitation
                reg_acc.append(np.nansum(reg))
                np.savetxt("reg.csv", reg, delimiter=",") #store revenue difference
                np.savetxt("reg_acc.csv", reg_acc, delimiter=",")
                ...
                reg_mis_acc.append(np.nansum(reg_mis))
                np.savetxt("reg_mis.csv", reg_mis, delimiter=",") #revenue difference of RMLP-2
                np.savetxt("reg_mis_acc.csv", reg_mis_acc, delimiter=",")
                if t==l_k:
                    print('--exploit--')
                t=t+1
