...
class Solution:
    ...
    def phi(self, x, thetax):
        return x-(-(2*x)**5/5+2*(2*x)**3/3-2*x+8/15)/(2*(1-(2*x)**2)**2)+thetax
    def F(self, x,thetax):
        if x-thetax>0.5:
            return 0
        elif x-thetax<-0.5:
...
for T in T_l:
    epi_num=math.ceil(math.log(T/l_0+1, 2)) #Number of Episodes
    reg_T=[]
    reg_T_tt=[]
    reg_T_tf=[]
    reg_T_bt=[]
    for times in range(30):                                                         #Repeat 30 times
        ...
        for k in range(epi_num):
            t=0
            l_k=l_0*2**k
            l_k1=math.floor((l_k*3)**(5/7))                                         #Length of exporation phase #m=2#
            v_i=[]
            v_i_true=[]
            v_t=[]
            y_t=[]
            ...
            while t<=l_k:
                if t<=l_k1:                                                                     #Generate Data
                    ...
                    p_t_coll.append(p_t)
                    op_price=solu.phi_root(-0.48,0.48,theta_x)+theta_x                          #optimal price
                    rev_diff=solu.F(op_price,theta_x)-solu.F(p_t,theta_x)                       #revenue difference in exploration phase
                    print(rev_diff)
                    ...
                    np.savetxt("reg1.csv", reg, delimiter=",")                                   #regret in our regime
                    ...
                    v_t.append(v)
                    if t==l_k1:                                                                 #Update Parameters, theta, F#
                        ...
                        theta1=np.dot(inv(np.dot(X_1.transpose(),X_1)),np.dot(X_1.transpose(),B*y_t)) #Update parameter
                    ...
                    est_price=ker.phi_root(np.log(-(true-0.3)/(true+1.3)),theta_est,0.5)+theta_est #post price
                    rev_diff=op_rev-solu.F(est_price,theta_x) #our revenue difference
                    print(rev_diff)
                    ...
                    reg.append(rev_diff)                                # our regret in exploitation phase
                    reg_tt.append(rev_diff_tt)                          #regret with known theta
                    reg_tf.append(rev_diff_tf)                          #regret with known F
                    reg_bt.append(0)
        ...
        reg_tf=reg_tf[0:T]
        reg_bt=reg_bt[0:T]
        reg_T.append(np.nansum(reg))                                    # append all regret together
        reg_T_tt.append(np.nansum(reg_tt))
        reg_T_tf.append(np.nansum(reg_tf))
        reg_T_bt.append(np.nansum(reg_bt))
        np.savetxt("reg_t1.csv", np.array(reg_T), delimiter=",")        #save as a csv file for every repeatation
    ...
    reg_Tcum=np.vstack([reg_Tcum,reg_T.reshape(1,30)])
    ...
    #print(reg_Tcum)
    np.savetxt("reg_tcum1.csv", reg_Tcum, delimiter=",")                 #30 times simulation in a csv file.
