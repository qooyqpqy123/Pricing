...
for T in T_l:
    epi_num=math.ceil(math.log(T/l_0+1, 2))#Episode length
    ...
    for times in range(30):
        ...
        for k in range(epi_num):
            t=0
            l_k=l_0*2**k
            l_k1=math.floor((l_k*3)**(9/15))#Length of exporation phase #m=4#
            v_i=[]
            v_i_true=[]
            v_t=[]
            y_t=[]
            ...
            while t<=l_k:
                if t<=l_k1:  #Exploration Phase
                    ...
                    np.savetxt("reg1.csv", reg, delimiter=",")#regret in our regime in exploration phase
                    ...
                    reg.append(rev_diff)#exploitation regret in our regime
                    reg_tt.append(rev_diff_tt)
                    reg_tf.append(rev_diff_tf)
                    reg_bt.append(0)
                    np.savetxt("reg1.csv", reg, delimiter=",") #store in a csv file
                    np.savetxt("regtt.csv", reg_tt, delimiter=",")
                    np.savetxt("regtf.csv", reg_tf, delimiter=",")
                    np.savetxt("regbt.csv", reg_bt, delimiter=",")
                    if t==l_k:
    ...
    #print(reg_Tcum)
    np.savetxt("reg_tcum1.csv", reg_Tcum, delimiter=",")# 30 times simulations stored.
