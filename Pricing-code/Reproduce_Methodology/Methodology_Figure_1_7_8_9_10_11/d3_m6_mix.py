...
########Basic Model Setup with strong mixing features, See section 5 of the paper for more details##########
...
for T in T_l:
    ...
    for times in range(30):#Repeat 30 times
        ...
        for k in range(epi_num):
            t=0
            l_k=l_0*2**k
            l_k1=math.floor((l_k*3)**(13/23)) #Length of exporation phase #m=6#
            v_i=[]
            v_i_true=[]
            v_t=[]
            y_t=[]
            ...
            while t<=l_k: #Exploration phase
                if t<=l_k1:  
                    ...
                    np.savetxt("reg1.csv", reg, delimiter=",") #Record regret in exploration phase
                    ...
                    est_price=ker.phi_root(np.log(-(true-0.3)/(true+1.4)),theta_est,0.5)+theta_est
                    rev_diff=op_rev-solu.F(est_price,theta_x)#our regret
                    ...
                    reg.append(rev_diff)#exploitation regret
                    reg_tt.append(rev_diff_tt)
                    reg_tf.append(rev_diff_tf)
                    reg_bt.append(0)
                    np.savetxt("reg1.csv", reg, delimiter=",") #recored regret in exploitation phase
                    np.savetxt("regtt.csv", reg_tt, delimiter=",")
                    np.savetxt("regtf.csv", reg_tf, delimiter=",")
                    np.savetxt("regbt.csv", reg_bt, delimiter=",")
                    if t==l_k:
    ...
    reg_Tcum=np.vstack([reg_Tcum,reg_T.reshape(1,30)])#write all repeatations 30 times in a csv file
